<template>
  <div class="webtablevue">
      ...
      <template v-for="(item, index) of columns">
        ...
        <el-table-column v-if=" item.type === 'text' " :key="index" :fixed="item.fixed" :prop="item.prop" :type="item.type" :align="item.align ? item.align : 'center'" :width="item.width">
          ...
          <template slot-scope="scope">
            <el-button plain v-for="(item1, i) in item.list" :key="i" @click="item1.click" type="text" :size="item.size || 'mini'">{{ item1.label }}</el-button>
        ...
        <el-table-column v-else-if="item.type === 'select'" :key="index" :fixed="item.fixed" :label="item.label" :align="item.align" :width="item.width">
          <template slot="header" slot-scope="scope">
            <slot name="header" :item="item"> {{item.label}}</slot>
          </template>
          <template slot-scope="scope">
        ...
        <el-table-column v-else-if="item.type === 'selection'" :key="index" :fixed="item.fixed" :prop="item.prop" type="selection" :label="item.label" :align="item.align" :width="item.width">
        </el-table-column>

        <el-table-column v-else-if="item.type === 'button'" :key="index" :fixed="item.fixed" :prop="item.prop" :label="item.label" :align="item.align ? item.align : 'center'" :width="item.width">
          <template slot="header" slot-scope="scope">
            <slot name="header" :item="item"> {{item.label}}</slot>
          </template>
          ...
          <template slot-scope="scope" style="text-align: right">
            <!-- <el-button-group> -->
            <el-button plain v-for="(item2, i) in item.list" :key="i" @click="item2.click(scope.$index, scope.row)" :size="item.size || 'mini'" :type="item2.type" v-show="
        ...
        <el-table-column v-else-if="item.type === 'index'" :label="item.label" :type="item.type" :key="index" :width="item.width">
          <template slot-scope="scope">
            {{
        ...
        <el-table-column v-else :label="item.label" :key="index" :fixed="item.fixed" :prop="item.prop" :align="item.align ? item.align : 'center'" :width="item.width" :type="item.type">
          <template slot="header" slot-scope="scope">
            <slot name="header" :item="item"> {{item.label}}</slot>
          </template>
          <template slot-scope="scope">
    ...
    <el-pagination :page-sizes="pagesizes" layout="total,sizes, prev, pager, next,jumper" v-show="total > 0" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="currentpage" :page-size.sync="pagesize" :total="total">
    </el-pagination>
  </div>
</template>
<script>
